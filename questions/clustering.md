...
> GMMs are great to mimic general probilistic distributions, and are hence frequently used in [Bayesian modelling](https://en.wikipedia.org/wiki/Mixture_model#Examples).
...
> The obvious case is when you do not have labels.
...
1. Can you kernelize the data in GMMs as you did with the k-means? Would that help in the number of components the GMMs need?
  > I have not ried to do so myself, but [others](https://arxiv.org/abs/1608.00550) seem to do so.

1. In the section on k-means, VanderPlas states that 'undertypical circumstances, each repetition of the E-step and M-step will always result in a better estimate of the cluster characteristics.' What would be an untypical scenario in this case? Moreover, does a practical maximum number Expectation-Maximisation cycles exist, after which the process is ended without reasonable cluster generation  (when employing E-M on a dataset witout any apparent cluster, for instance equidistantly ordered items )?
...
> If there are more students that want us to include the theory of Dirichlet Processes in the exam, I can always do so. Let me know if that is the case.
...
1. If the k-means algorithm is not effective for clustering our data set, in which cases should we procceed with PCA and when with a Gaussian mixture model?
> FIXME
...
> I use a subset just for the purpose of executing rapidly. In practice you probably would like to use as much data as possible.
...
> None of the algorithms is ideal.
