...
1. When we combine different database in the latest generation of pathway analysis. How can we equally weight the data in different analysis or do we need to weight? I think we know that you can gain information from different role but each role might not be comparable to one another.
